public class Classroom {
    ...
    public boolean addStudent(Student student)
    {
        for (int i = 0; i < classRoster.length; i++) {
            String name = student.getFirst() + student.getLast();
            if (i == classRoster.length -1 ) {
                return false;
            }
            else classRoster[i] = student;
            return true;
        }
    }
    ...
    public double classExamAverage()
    {
        int totalScore = 0;
        for (int i = 0; i < classRoster.length; i++) {
            int score = Student.getExamScores();
            totalScore += score;
        }
    }
    ...
    {
        for (int i = 0; i < classRoster.length; i++) {
            System.out.println(toString(classRoster[i]));
        }
    }
}
